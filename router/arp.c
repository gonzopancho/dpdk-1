                    ...
                    struct arp_hdr* arphdr)
{
  ...
  if (!is_own_ip_addr(arp_hdr->arp_tip)) return 0;
  
  res = add_arp_table_entry(arp_table, &body->arp_sip, &body->arp_sha);
  if (res) {
    RTE_LOG(ERR, ARP_TABLE, 
...
arp_reply_process(struct rte_mbuf* buf)
{
  ...
  if (!is_same_ether_addr(&arp_data->arp_tha, &etheraddr)) return 0;
...
arp_input(struct rte_mbuf* buf)
{
  ...
  struct arp_hdr* arp_hdr;
  arp_hdr = (struct arp_hdr *) (rte_pktmbuf_mtod(buf, char*) + buf->l2_len);
  if (arp_hdr->arp_hrd != ARP_HRD_ETHER) return 0;
